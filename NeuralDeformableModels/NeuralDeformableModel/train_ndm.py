...
def main():
    ...
    pretrained = False
    pretrained_model = '650_0.0107_model.pth'
    use_pretrained = True
    ...
    while use_pretrained:
        ...
        samples_in_epoch_num = 800

        nbatch = int(samples_in_epoch_num / batchsize)

        for epoch in range(start_epoch, 5000):

            print('Set the t_optimizer')
            if epoch < 100 * it_num + 1:
                optimizer = optim.Adam(list(GeoMorph.parameters()), lr=5e-4)  # 5e-4
            else:
                optimizer = optim.Adam(list(GeoMorph.parameters()), lr=1e-5)  # 5e-4

            mean_radial_distance = 0
            logger.info('Epoch %d (%d/%s):' % (global_epoch + 1, epoch + 1, opt.num_epochs))

            '''learning one epoch'''
            samples_in_epoch = np.random.permutation(samples_in_epoch_num)
            samples_in_epoch = samples_in_epoch.reshape((-1, batchsize))
            sw = 0.2

            for i in range(nbatch):
                samples = samples_in_epoch[i, :]
                bs = 0
                m_pc = np.zeros([batchsize, 5600, 3])
                epi_myo_gt = np.zeros([batchsize, 3000, 3])
                endo_myo_gt = np.zeros([batchsize, 3000, 3])
                rv_gt = np.zeros([batchsize, 3000, 3])
                ...
                while (bs < batchsize):
                    ...
                    m_pc[bs, ::] = np.expand_dims(m_pc0, axis=0)
                    epi_myo_gt[bs, ::] = np.expand_dims(epi_myo_gt0, axis=0)
                    endo_myo_gt[bs, ::] = np.expand_dims(endo_myo_gt0, axis=0)
                    rv_gt[bs, ::] = np.expand_dims(rv_gt0, axis=0)
                    bs += 1
                ...
                if epoch < it_num + 1:
                    ...
                    sm_loss1 = quaternion_loss(quaternion, quaternion2) + quaternion_loss(scale, scale2) + \
                               quaternion_loss(trans, trans2)
                    ...
                    sm_loss1 = smooothing_loss(a3) + quaternion_loss(quaternion, quaternion2) + \
                               quaternion_loss(scale, scale2) + quaternion_loss(trans, trans2)
                    ...
                    sm_loss1 = smooothing_loss(e1) + smooothing_loss(e2) + smooothing_loss(a3) + quaternion_loss(
                        quaternion,
                        quaternion2) + quaternion_loss(
                        scale, scale2) + quaternion_loss(trans, trans2)
                    sm_loss2 = smooothing_loss(e12) + smooothing_loss(e22) + smooothing_loss(a32) + quaternion_loss(
                        quaternion,
                        quaternion3)
                    ...
                    endo_pred0, endo_pred1 = GeoMorph.neural_mesh_forward2(code2.detach(), endo_pc.detach(),
                                                                           endo_myo_gt)
                ...
                mean_radial_distance += loss.item()
                # mean_radial_distance2 += p2s_loss.item()
                logger.info('Train sample loss is: %.5f' % loss)

            train_mean_radial_distance = mean_radial_distance / nbatch
            logger.info('Train radial loss is: %.5f' % train_mean_radial_distance)
            train_loss_dict.append(train_mean_radial_distance)
            np.savetxt(os.path.join(model_path, 'train_loss.txt'), train_loss_dict, fmt='%.6f')

            if (epoch) % 10 == 0:
                torch.save(GeoMorph.state_dict(),
                           os.path.join(model_path, '{:d}_{:.4f}_model.pth'.format(epoch, train_mean_radial_distance)))
            pretrained_model = '{:d}_{:.4f}_model.pth'.format(epoch, train_mean_radial_distance)
            pretrained = True
