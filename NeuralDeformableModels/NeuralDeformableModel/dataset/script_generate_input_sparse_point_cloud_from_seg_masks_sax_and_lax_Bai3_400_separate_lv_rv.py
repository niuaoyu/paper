...
def determin_epi_endo_points(points, np_mask_img):
    # epi: 1
    # endo: 2
    s = points.shape
    myo_pc = np.ones([s[0], 3])
    myo_pc[:, 0:2] = points
    ...
    for k in range(s[0]):
        ...
        while (comparison):
            if px - ks < 0 or px + ks > size[0] - 1 \
                ...
                comparison = False
                continue
            lv_kernel = lv[px - ks: px + ks + 1, py - ks: py + ks + 1]
            myo_kernel = myo[px - ks: px + ks + 1, py - ks: py + ks + 1]
...
def collect_surfaces(m_pc):
    epi = []
    endo = []
    rv = []
    for s in range(m_pc.shape[0]):
        if m_pc[s, 3] < 1.5:
            epi.append(m_pc[s, :3])
        elif m_pc[s, 3] < 2.5:
            endo.append(m_pc[s, :3])
        else:
            rv.append(m_pc[s, :3])

    return np.array(epi), np.array(endo), np.array(rv)




src_root1 = '/ailab/data/Cardiac super-resolution label maps/smoothed/'
dst_root = '/ailab/data/Cardiac super-resolution label maps/pc3/'

endo_root = '/ailab/data/Cardiac super-resolution label maps/Dataset/'
...
for subroot, dirs, files in os.walk(src_root1):
    for file in files:
        if file.endswith('nii.gz') and 'MYO' in file:
            ...
            mask_file = os.path.join(os.path.join(subroot.replace('smoothed', 'Dataset'), file.replace('_MYO.', '.')))
            mask_image = sitk.ReadImage(mask_file)
            size = mask_image.GetSize()  # x, y, z
            endo = sitk.GetArrayFromImage(mask_image)
            ...
            for s in range(m):
                ...
                if len(contours) > 1:
                    ...
                    if c_endo.shape[0] > 20:
                        ...
                        m_pc1[s, :(lv_num-n_endo), 3] = determin_epi_endo_points(c_epi_1, np_mask_img)
                        m_pc1[s, :, 2] = z
                        m_pc1[s, (lv_num-n_endo):lv_num, :2] = c_endo_1
                        m_pc1[s, (lv_num-n_endo):lv_num, 3] = determin_epi_endo_points(c_endo_1, np_mask_img)
                    else:
                        c_epi = np.asarray(contours[c_ind[-1]])
                        c_epi_1 = farthest_point_sample(c_epi, npoint=lv_num)
                        c_epi_1 = c_epi[c_epi_1]
                        m_pc1[s, :lv_num, :2] = c_epi_1
                        m_pc1[s, :lv_num, 3] = determin_epi_endo_points(c_epi_1, np_mask_img)
                        m_pc1[s, :, 2] = z
                ...
                if lv_num < 201:
                    ...
                    if len(contours) > 1:
                        ...
                        if c_endo.shape[0] > 20:
                            ...
                            m_pc1[s, lv_num:(lv_num + lv_num - n_endo), 3] = 3
                        else:
                            c_epi = np.asarray(contours[c_ind[-1]])
                            c_epi_1 = farthest_point_sample(c_epi, npoint=lv_num)
                            c_epi_1 = c_epi[c_epi_1]
            ...
            # ax.scatter(m_pcx[:4000, 0], m_pcx[:4000, 1], m_pcx[:4000, 2], s=2,  c=m_pcx[:4000, 3], cmap='viridis')
            # ax.axis()
            # plt.show()
                ...
                if s != m:
                    ...
                    if len(contours) > 1:
                        ...
                        if c_endo.shape[0] > 20:
                            ...
                            m_pc1[s, (lv_num - n_endo):, 3] = determin_epi_endo_points(c_endo_1, np_mask_img)
                            ...
                            m_pc1[s, :, 3] = determin_epi_endo_points(c_epi_1, np_mask_img)
            ...
            # fig = plt.figure(figsize=(3, 3))
            # ax = fig.add_subplot(111, projection='3d')
            # ax.scatter(m_pc[:, 0], m_pc[:, 1], m_pc[:, 2], s=2,  c=m_pc[:, 3], cmap='viridis')
            ...
            pc_np_file = os.path.join(save_dir, file.replace('MYO.nii.gz', 'pc1.npz'))
            np.savez_compressed(pc_np_file, m_pc)

            m_pc, epi_myo, endo_myo, rv = pc_normalize(m_pc, epi_myo, endo_myo, rv)

            m_pc_color[:, :3] = m_pc
            m_pc_color[:, 3] = m_pc_color[:, 3]

            m_pc = m_pc_color

            epi_input, endo_input, rv_input = collect_surfaces(m_pc)

            epi_input_ind = farthest_point_sample(epi_input, npoint=1800)
            endo_input_ind = farthest_point_sample(endo_input, npoint=1800)
            rv_input_ind = farthest_point_sample(rv_input, npoint=1800)

            epi_input = epi_input[epi_input_ind]
            endo_input = endo_input[endo_input_ind]
            rv_input = rv_input[rv_input_ind]

            # fig = plt.figure(figsize=(3, 3))
            # ax = fig.add_subplot(111, projection='3d')
            # ax.scatter(epi_myo[:, 0], epi_myo[:, 1], epi_myo[:, 2], s=2, c='red')
            # ax.scatter(epi_input[:, 0], epi_input[:, 1], epi_input[:, 2], s=2, c='blue')
            # ax.axis()
            # plt.show()
            #
            # fig = plt.figure(figsize=(3, 3))
            # ax = fig.add_subplot(111, projection='3d')
            # ax.scatter(endo_myo[:, 0], endo_myo[:, 1], endo_myo[:, 2], s=2, c='red')
            # ax.scatter(endo_input[:, 0], endo_input[:, 1], endo_input[:, 2], s=2, c='blue')
            # ax.axis()
            # plt.show()
            #
            # fig = plt.figure(figsize=(3, 3))
            # ax = fig.add_subplot(111, projection='3d')
            # ax.scatter(rv[:, 0], rv[:, 1], rv[:, 2], s=2, c='red')
            # ax.scatter(rv_input[:, 0], rv_input[:, 1], rv_input[:, 2], s=2, c='blue')
            # ax.axis()
            # plt.show()
            ...
            pc_np_file = os.path.join(save_dir, file.replace('MYO.nii.gz', 'epi_pc.npz'))
            np.savez_compressed(pc_np_file, epi_input)

            pc_np_file = os.path.join(save_dir, file.replace('MYO.nii.gz', 'endo_pc.npz'))
            np.savez_compressed(pc_np_file, endo_input)

            pc_np_file = os.path.join(save_dir, file.replace('MYO.nii.gz', 'rv_pc.npz'))
            np.savez_compressed(pc_np_file, rv_input)
