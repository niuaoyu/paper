...
dst_root = '/ailab/data/Cardiac super-resolution label maps/processed'
...
for subroot, dirs, files in os.walk(src_root1):
    for file in files:
        if file.endswith('nii.gz') and 'MYO' in file:
            ...
            """
            mask_image = sitk.ReadImage(os.path.join(subroot, file))
            np_imgs = sitk.GetArrayFromImage(mask_image)
            ...
            """
            x_min = myo_data[:, 0].min() - 1
            x_max = myo_data[:, 0].max()- 1

            y_min = myo_data[:, 1].min()- 1
            y_max = myo_data[:, 1].max()- 1

            z_min = myo_data[:, 2].min()- 1
            z_max = myo_data[:, 2].max()- 1
            """
            ...
            data_root1 = os.path.join(subroot, file.replace('.nii.gz', '_epi2.npz'))
            np.savez_compressed(data_root1, epi)

            data_root1 = os.path.join(subroot, file.replace('.nii.gz', '_endo2.npz'))
            np.savez_compressed(data_root1, endo)
