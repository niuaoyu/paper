...
class PointNetSetAbstraction2(nn.Module):
    def __init__(self, radius, nsample, in_channel, mlp, group_all, knn=False, norm_type="bn"):
        super().__init__()
        self.radius = radius
        self.nsample = nsample
        self.knn = knn
        ...
        def make_norm(c: int):
            if norm_type == "bn":
                ...
                raise ValueError(f"Unknown norm_type: {norm_type}. Use 'bn' or 'in'.")

        last_channel = in_channel
        for out_channel in mlp:
            self.mlp_convs.append(nn.Conv2d(last_channel, out_channel, 1))
            self.mlp_bns.append(make_norm(out_channel))
            last_channel = out_channel
    ...
    def forward(self, xyz, points):
        ...
        if self.group_all:
            ...
            npoint = xyz.shape[1] // 4
            new_xyz, new_points = sample_and_group(npoint, self.radius, self.nsample, xyz, points, knn=self.knn)
        # new_xyz: sampled points position data, [B, npoint, C]
        # new_points: sampled points data, [B, npoint, nsample, C+D]
        new_points = new_points.permute(0, 3, 2, 1)  # [B, C+D, K, S]
        for conv, norm in zip(self.mlp_convs, self.mlp_bns):
            new_points = F.relu(norm(conv(new_points)))
...
class PointNetFeaturePropagation(nn.Module):
    def __init__(self, in_channel, mlp, norm_type: str = "bn"):
        super(PointNetFeaturePropagation, self).__init__()
        self.mlp_convs = nn.ModuleList()
        self.mlp_bns = nn.ModuleList()  # BN or IN

        def make_norm(c: int) -> nn.Module:
            if norm_type == "bn":
                return nn.BatchNorm1d(c)
            elif norm_type == "in":
                # InstanceNorm1d normalizes per-sample per-channel over the length dimension (N)
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        # 3-NN interpolation (your code has a nonstandard line: if S>2: S=3)
        if S == 1:
            interpolated_points = points2.repeat(1, N, 1)  # [B, N, D]
        else:
            if S > 2:
                S = 3  # keep your original behavior
            dists = square_distance(xyz1, xyz2)  # [B, N, S_original]

            dists, idx = dists.sort(dim=-1)
            dists, idx = dists[:, :, :S], idx[:, :, :S]  # [B, N, S]

            dist_recip = 1.0 / (dists + 1e-8)            # [B, N, S]
            norm = torch.sum(dist_recip, dim=2, keepdim=True)
            weight = dist_recip / norm                    # [B, N, S]

            interpolated_points = torch.sum(
                index_points(points2, idx) * weight.view(B, N, S, 1),
                dim=2
            )  # [B, N, D]
