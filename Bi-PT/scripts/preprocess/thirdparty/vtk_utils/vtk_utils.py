...
def map_polydata_coords(poly, displacement, transform, size):
    ...
    # Flip x and y to match VTK image coord system
    coords[:,0] = size[0]-1-coords[:,0]
    coords[:,1] = size[1]-1-coords[:,1]
...
def vtk_surface_nets_multi(vtkLabel, bg_id):
    """
    SurfaceNets equivalent of vtk_marching_cube_multi(): extracts all labels except bg_id.
    """
...
def _largest_region(poly, min_cells=0):
    """Keep the largest connected region in a vtkPolyData.
       If min_cells>0, drop regions smaller than this after picking largest."""
    ...
    if min_cells > 0 and out.GetNumberOfCells() < min_cells:
        # Return an empty polydata with same structure
        empty = vtk.vtkPolyData()
        empty.DeepCopy(out)  # copies structure; we’ll clear it
        empty.Initialize()
        return empty
    return out
...
#         p = decimation(p, rate)
#         p = cleanPolyData(p, 0.)
#         # p = _largest_region(p, min_cells=50)
#         n_points = p.GetNumberOfPoints()
#         region_ids_np = np.full((n_points, 2), i, dtype=np.int32)
#         region_ids_vtk = numpy_to_vtk(region_ids_np, deep=True)
#         region_ids_vtk.SetName('RegionId')
#         p.GetPointData().AddArray(region_ids_vtk)
...
def convert_to_surfs(seg, new_spacing=[1.,1.,1.], target_node_num=2048, bound=False):
    ...
    seg_vtk,_ = exportSitk2VTK(seg2)
    seg_vtk = vtkImageResample(seg_vtk,new_spacing,'NN')
    p = vtk_surface_nets_multi(seg_vtk, 0)
    p = smooth_polydata(p, iteration=50)
    rate = max(0., 1. - float(target_node_num)/float(p.GetNumberOfPoints()))
    p = decimation(p, rate)
    p = cleanPolyData(p, 0.0)
    poly = p
    if bound:
        poly = bound_polydata_by_image(seg_vtk, poly, 1.5)
    return poly
